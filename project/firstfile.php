...
<head>
<meta charset="UTF-8">
<title class="mx-auto" style="width: 200px;">Login</title>
...
if($user_type=='admin'){
    try{
        $sql="SELECT * FROM shopify.admin WHERE email = '".$email."'";
    ...
    try{
        $sql="SELECT * FROM shopify.customercare WHERE email = '".$email."'";
        $stmt=$conn->prepare($sql);
        if ($stmt->execute()){
            $count=$stmt->rowCount();
            if($count==0){
