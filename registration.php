...
<?php } else { ?>
You are already logged in. Click <a href="../project/logout.php">here</a> to logout.
<?php }
?>
</center>
...
if($user_type=='seller'){
    try{
        $sql="SELECT * FROM shopify.seller WHERE email = '".$email."'";
    ...
    try{
        $sql="INSERT INTO shopify.users (userid,email, pass) VALUES (:userid,:email, :pass)";
        $stmt=$conn->prepare($sql);
        $stmt->execute(array(
            ':userid' =>$_POST['userid'],
            ':email' =>$_POST['email'],
        ...
        $sql2="INSERT INTO shopify.seller (sellerid,sname,email,pass,occupation,address,contactNo) VALUES (:sellerid,:sname,:email,:pass,:occupation,:address,:contactNo)";
    ...
    echo "$pass";
    try{
        $sql="SELECT * FROM shopify.buyer WHERE email = '".$email."'";
        $stmt=$conn->prepare($sql);
        if ($stmt->execute()){
            $count=$stmt->rowCount();
            if($count>0){
    ...
    try{
        ...
        $sql2="INSERT INTO shopify.buyer (buyerid,bname,email,pass,occupation,address,contactNo) VALUES (:buyerid,:bname,:email,:pass,:occupation,:address,:contactNo)";
        $stmt2=$conn->prepare($sql2);
        $stmt2->execute(array(
        ...
        $sql3="INSERT INTO shopify.cart (buyerid) VALUES (:buyerid)";
